#!/bin/bash
# #1249's Linux control harness: #1257's shape unchanged (one run, one fresh directory reaped
...
# --cpus=2, --network none), mounting this worktree. It exists to show that the Windows
# compile fix on branch issue-1249 leaves the Linux score stream where #817 left it.
#
...
mkdir -p "$BASE"
if [ -d "$RUN" ]; then
  od_run "i1249-clean-$PHASE" --network none -v "$BASE":/base "$OD_IMAGE" \
...
od_run "i1249-$PHASE" --cpus=2 --network none -e HOME=/root \
  -v "$OD_TREE_ROOT":/app \
