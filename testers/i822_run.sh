...
if [ -d "$RUN" ]; then
  docker run --rm --name "$(od_name "822-clean-$LABEL")" -v "$BASE":/base "$OD_IMAGE" \
...
docker run --rm --name "$(od_name "822-$LABEL")" --cpus=3 -e HOME=/root \
  -v "$OD_TREE_ROOT":/app \
  -v "$RUN":/run822 -v "$RUN/cfg":/root/.config \
  -w /run822 "$OD_IMAGE" bash /run822/inside.sh
RC=$?
