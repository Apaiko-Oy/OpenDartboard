#!/bin/bash
# #1451's harness: #1449's shape with its own paths. One run, one fresh directory reaped
...
# stub listens.
#
#   testers/i1451_run.sh [container-bash-script-file]
#
...
HERE="$OD_TREE_ROOT"
SCRIPT="${1:-$HERE/testers/phases1451/1451-scoring.sh}"
PHASE="$(od_phase "$SCRIPT")"
BASE="$OD_RUNS_BASE/1451"
RUN="$BASE/$PHASE"
mkdir -p "$BASE"
if [ -d "$RUN" ]; then
  od_run "1451-clean-$PHASE" --network none -v "$BASE":/base "$OD_IMAGE" \
...
od_run "1451-$PHASE" --cpus=2 --network none -e HOME=/root \
  -v "$HERE":/app \
  -v "$RUN":/run1451 -v "$RUN/cfg":/root/.config \
  -w /run1451 "$OD_IMAGE" bash /run1451/inside.sh
RC=$?
