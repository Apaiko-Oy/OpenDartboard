...
# twelve looks never reached a frame it could calibrate on, and that the opt-in budget
# (OD_LOOK_BUDGET=1605) does -- while the default stays #1445's, because admitting the
# camera in that window was measured to cost accuracy (testers/run_all.sh 1555: 15/23 ->
# 11/23; the darts are named at kFurtherLooksPastAStandingDart in geometry_detector.cpp).
#
...
#      not outlast the run -- the defect -- and the opt-in budget does.
#   B  THE BOARD UNDER OD_LOOK_BUDGET=1605, rig-20260922 dev: camera 1 calibrates on the
...
#   D  THE DEFAULT is main's: camera 1 set aside after twelve looks, SCORING 2 of 3, and
#      no background re-taken.
#   E  NOTHING ELSE MOVES: rig-20260918 in both windows and rig-20260922's opening give
#      byte-identical calibration transcripts with and without the opt-in, rig-20260918
...
echo "the budget this tree holds: $PINNED further looks by default, $BUDGET under OD_LOOK_BUDGET=1605, $SPACING capture cycles apart"
...
note $? "the opt-in $BUDGET looks do (first calibrating look ${FIRST_OK:-none})"
...
# The same transcript with the one sentence the opt-in is ALLOWED to change -- the budget
# printed in the LOOK AGAIN lines, and its own warning -- taken out.
comparable() {
  transcript "$1" | grep -v 'OD_LOOK_BUDGET=1605 is set' |
...
calibrate_once $R22 $RUN/r22-dev-1605.out  OD_LOOK_BUDGET=1605;          RC2=$?
calibrate_once $R22 $RUN/r22-open.out      OD_SEEK_VIDEO=off;            RC3=$?
calibrate_once $R22 $RUN/r22-open-1605.out OD_SEEK_VIDEO=off OD_LOOK_BUDGET=1605; RC4=$?
calibrate_once $R18 $RUN/r18-dev.out;                                    RC5=$?
calibrate_once $R18 $RUN/r18-dev-1605.out  OD_LOOK_BUDGET=1605;          RC6=$?
calibrate_once $R18 $RUN/r18-open.out      OD_SEEK_VIDEO=off;            RC7=$?
calibrate_once $R18 $RUN/r18-open-1605.out OD_SEEK_VIDEO=off OD_LOOK_BUDGET=1605; RC8=$?
for f in r22-dev r22-dev-1605 r22-open r22-open-1605 r18-dev r18-dev-1605 r18-open r18-open-1605; do
...
grep -q "OD_LOOK_BUDGET=1605 is set" $D; note $? "the opt-in run says it is opted in"
...
echo
echo "---- D. the default is main's ----"
echo "     PREDICTED: camera 1 is set aside after $PINNED further looks, SCORING: 2 of 3, no background re-taken"
! grep -q "OD_LOOK_BUDGET=1605 is set" $RUN/r22-dev.cal; note $? "the default run is not opted in"
grep -q "LOOK AGAIN: camera(s) 1 were refused on the averaged frame and on all $PINNED further looks" $RUN/r22-dev.cal
note $? "camera 1 refused on the averaged frame and on all $PINNED further looks"
[ "$(scoring_of $RUN/r22-dev.cal)" = "SCORING: 2 of 3" ]; note $? "SCORING: 2 of 3 -- the board this issue was filed on, unchanged"
! grep -q "its background is re-taken" $RUN/r22-dev.cal; note $? "and its background is the averaged calibration frames"

echo
echo "---- E. nothing else moves ----"
for pair in "r18-dev" "r18-open" "r22-open"; do
  comparable $RUN/$pair.out > $RUN/$pair.cmp
  comparable $RUN/$pair-1605.out > $RUN/$pair-1605.cmp
  if diff -q $RUN/$pair.cmp $RUN/$pair-1605.cmp > /dev/null; then
    note 0 "$pair: the calibration transcript is byte-identical with and without the opt-in ($(wc -l < $RUN/$pair.cmp) lines; $(scoring_of $RUN/$pair.cal))"
  else
    diff $RUN/$pair.cmp $RUN/$pair-1605.cmp | head -10
    note 1 "$pair: the calibration transcript moved with the budget"
  fi
  ! grep -q "its background is re-taken" $RUN/$pair-1605.cal; note $? "$pair: no background re-taken under the opt-in"
done
...
grep -oE 'LOOK AGAIN: camera [0-9]+ calibrated on look [0-9]+ of [0-9]+' $RUN/r22-dev-1605.cal | sed 's/^/     r22-dev-1605:  /'

echo
if [ "$FAIL" -gt 0 ]; then echo "RESULT: $FAIL failure(s)"; exit 1; fi
echo "RESULT: under OD_LOOK_BUDGET=1605 the looks outlast the dart that stood through camera 1's bull and camera 1 calibrates in the dev window with the opening's figures; the default is main's; nothing else moves"
exit 0
