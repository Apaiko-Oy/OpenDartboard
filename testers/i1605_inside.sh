...
# twelve looks never reached a frame it could calibrate on, and that the budget now does.
#
...
#      looks is refused, and then the camera calibrates. The pinned budget (#1445's) does
#      not outlast the run -- the defect -- and the tree's budget does.
#   B  THE BOARD, rig-20260922 dev: camera 1 calibrates on the look the census predicts,
#      SCORING 3 of 3, and every look is before the one seal (ADR-0080 section 2).
...
#   D  THE PIN: OD_LOOK_BUDGET=1445 on the same binary sets camera 1 aside after twelve
#      looks, as main did -- SCORING 2 of 3.
#   E  NOTHING ELSE MOVES: rig-20260918 in both windows and rig-20260922's opening give
#      byte-identical calibration transcripts with and without the pin, and rig-20260918
#      takes no look at all. The budget is only ever spent past look twelve, and only on
#      a camera still refused there.
#
...
BUDGET="$(grep -oE 'constexpr int kFurtherLooks = [0-9]+' $SRC | grep -oE '[0-9]+$')"
PINNED="$(grep -oE 'constexpr int kFurtherLooksAsOf1445 = [0-9]+' $SRC | grep -oE '[0-9]+$')"
...
echo "the budget this tree holds: $BUDGET further looks ($PINNED under OD_LOOK_BUDGET=1445), $SPACING capture cycles apart"
...
note $? "the tree's $BUDGET looks do (first calibrating look ${FIRST_OK:-none})"
...
comparable() {
  transcript "$1" | grep -v 'OD_LOOK_BUDGET=1445 is set' |
...
calibrate_once $R22 $RUN/r22-dev-pin.out  OD_LOOK_BUDGET=1445;          RC2=$?
calibrate_once $R22 $RUN/r22-open.out     OD_SEEK_VIDEO=off;            RC3=$?
calibrate_once $R22 $RUN/r22-open-pin.out OD_SEEK_VIDEO=off OD_LOOK_BUDGET=1445; RC4=$?
calibrate_once $R18 $RUN/r18-dev.out;                                   RC5=$?
calibrate_once $R18 $RUN/r18-dev-pin.out  OD_LOOK_BUDGET=1445;          RC6=$?
calibrate_once $R18 $RUN/r18-open.out     OD_SEEK_VIDEO=off;            RC7=$?
calibrate_once $R18 $RUN/r18-open-pin.out OD_SEEK_VIDEO=off OD_LOOK_BUDGET=1445; RC8=$?
for f in r22-dev r22-dev-pin r22-open r22-open-pin r18-dev r18-dev-pin r18-open r18-open-pin; do
...
echo
echo "---- B. rig-20260922, dev window, the tree's budget ----"
echo "     PREDICTED: camera 1 calibrates on look ${FIRST_OK:-?} of $BUDGET (the census's first clean look), SCORING: 3 of 3"
LOOKED="$(grep -oE 'LOOK AGAIN: camera 1 calibrated on look [0-9]+ of [0-9]+' $RUN/r22-dev.cal | head -1)"
echo "     measured:  ${LOOKED:-no such line}; $(scoring_of $RUN/r22-dev.cal); $(seal_cam1 $RUN/r22-dev.cal)"
...
[ "$(scoring_of $RUN/r22-dev.cal)" = "SCORING: 3 of 3" ]; note $? "and the board scores with 3 of 3 cameras"
seal_cam1 $RUN/r22-dev.cal | grep -q 'scoring=1'; note $? "camera 1 is in the seal, scoring"
SEALS="$(grep -c 'GEOMETRY SEALED:' $RUN/r22-dev.cal)"
SL="$(grep -n 'GEOMETRY SEALED:' $RUN/r22-dev.cal | head -1 | cut -d: -f1)"
LAST="$(grep -n 'LOOK AGAIN' $RUN/r22-dev.cal | tail -1 | cut -d: -f1)"
...
WD="$(cam1_last $RUN/r22-dev.cal 'Camera 1 wire model: [0-9]+ candidates')"
...
BD="$(cam1_last $RUN/r22-dev.cal 'Camera 1 bull at' | sed -n 's/.*bull at (\([0-9]*\),\([0-9]*\)).*/\1 \2/p')"
BO="$(cam1_last $RUN/r22-open.cal 'Camera 1 bull at' | sed -n 's/.*bull at (\([0-9]*\),\([0-9]*\)).*/\1 \2/p')"
RINGD="$(cam1_last $RUN/r22-dev.cal 'Camera 1 rings:')"
MODELD="$(cam1_last $RUN/r22-dev.cal 'Camera 1 board model')"
echo "     dev:     R=${RD:-none} bull=(${BD:-none}) ${RINGD#*- }"
echo "     opening: R=${RO:-none} bull=(${BO:-none})"
echo "     dev board model: ${MODELD#*- }" | cut -c1-400
echo "     the bulls camera 1 read while refused, pinned run: $(grep -oE 'Camera 1 bull at \([0-9]+,[0-9]+\)' $RUN/r22-dev-pin.cal | grep -oE '\([0-9]+,[0-9]+\)' | sort | uniq -c | tr '\n' ' ')"
...
echo
echo "---- D. the pin: OD_LOOK_BUDGET=1445 on the same binary ----"
echo "     PREDICTED: camera 1 is set aside after $PINNED further looks, SCORING: 2 of 3, as on main"
grep -q "OD_LOOK_BUDGET=1445 is set" $RUN/r22-dev-pin.cal; note $? "the pinned run says it is pinned"
grep -q "LOOK AGAIN: camera(s) 1 were refused on the averaged frame and on all $PINNED further looks" $RUN/r22-dev-pin.cal
note $? "camera 1 refused on the averaged frame and on all $PINNED further looks"
[ "$(scoring_of $RUN/r22-dev-pin.cal)" = "SCORING: 2 of 3" ]; note $? "SCORING: 2 of 3 -- the board this issue was filed on"

echo
echo "---- E. nothing else moves ----"
for pair in "r18-dev" "r18-open" "r22-open"; do
  comparable $RUN/$pair.out > $RUN/$pair.cmp
  comparable $RUN/$pair-pin.out > $RUN/$pair-pin.cmp
  if diff -q $RUN/$pair.cmp $RUN/$pair-pin.cmp > /dev/null; then
    note 0 "$pair: the calibration transcript is byte-identical with and without the pin ($(wc -l < $RUN/$pair.cmp) lines; $(scoring_of $RUN/$pair.cal))"
  else
    diff $RUN/$pair.cmp $RUN/$pair-pin.cmp | head -10
...
echo "RESULT: the budget outlasts the dart that stood through camera 1's bull, camera 1 calibrates in the dev window with the opening's figures, and nothing else moves"
exit 0
