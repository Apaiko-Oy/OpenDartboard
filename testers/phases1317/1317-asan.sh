...
# issue is about a bounds read. abort_on_error keeps the report and the exit code together.
...
for name in control partial; do
  ...
  grep -E 'ERROR: AddressSanitizer|SUMMARY: AddressSanitizer|runtime error' \
    /run1317/asan_$name.out /run1317/asan_$name.err || true
  N=$(cat /run1317/asan_$name.out /run1317/asan_$name.err | grep -cE 'ERROR: AddressSanitizer' || true)
...
mkdir -p $BASE_SRC
git -C /app archive "${BASE_COMMIT:-50e3b07}" | tar -x -C $BASE_SRC || exit 1
...
grep -E 'ERROR: AddressSanitizer|SUMMARY: AddressSanitizer' \
  /run1317/asan_base_partial.out /run1317/asan_base_partial.err || echo "(none)"
