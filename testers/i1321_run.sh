...
set -u
SCRIPT="${1:-/home/mikko/opendartboard/i1321/testers/phases1321/1321-reason.sh}"
BASE=/home/mikko/opendartboard/runs1321
RUN="$BASE/reason"
mkdir -p "$BASE"
if [ -d "$RUN" ]; then
  docker run --rm --name od-i1321-clean --network none -v "$BASE":/base od-amd64:bullseye \
...
docker run --rm --name od-i1321-reason --cpus=2 --network none -e HOME=/root \
  -v /home/mikko/opendartboard/i1321:/app \
  -v "$RUN":/run1321 -v "$RUN/cfg":/root/.config \
  -w /run1321 od-amd64:bullseye bash /run1321/inside.sh
RC=$?
